...
import type { CoverFilters as CoverFilterValues } from "@/features/covers/types";

type CoverFiltersProps = {
  value: CoverFilterValues;
  onChange: (filters: CoverFilterValues) => void;
  onSearch: () => void;
  onReset: () => void;
};

const sideOptions = ["", "BUY", "SELL"];
const statusOptions = ["", "PENDING", "FILLED", "FAILED", "SKIPPED"];
const modeOptions = ["", "FULL", "THRESHOLD", "DISABLED"];
const resultOptions = ["", "SUCCESS", "WAITING", "REJECTED", "SKIPPED"];

export function CoverFilters({ value, onChange, onSearch, onReset }: CoverFiltersProps) {
  ...
  return (
    <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">口座、通貨ペア、売買区分、カバー状態で履歴を絞り込みます。</p>
      </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        <label className="space-y-2">
          <span className="text-sm font-medium text-slate-700">coverMode</span>
          <select
            name="coverMode"
          ...
          >
            {modeOptions.map((mode) => (
              <option key={mode || "all"} value={mode}>
                {mode || "すべて"}
              </option>
            ))}
          </select>
        ...
        <label className="space-y-2">
          <span className="text-sm font-medium text-slate-700">executionResult</span>
          <select
