...
export async function getAdminTrades(filters: TradeFilters = defaultTradeFilters) {
  const response = await apiClient<TradesResponse>("/admin/trades", {
    query: {
  ...
  const items = applyFilters(response.items, filters);
  return {
    items,
    total: items.length,
  };
